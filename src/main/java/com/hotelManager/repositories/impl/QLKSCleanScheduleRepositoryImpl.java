...
public class QLKSCleanScheduleRepositoryImpl implements QLKSCleanScheduleRepository {
    ...
    @Override
    public List<QLKSCleanScheduleEntity> getAllViewRoom(GetViewCleanScheduleRequest request) throws HotelManagerException {
        Session session = sessionFactory.openSession();
        try {
            StringBuilder hql = new StringBuilder()
                    .append("FROM QLKSCleanScheduleEntity r ")
                    .append("WHERE 1 = 1 ")
                    .append("AND r.idEmployee = :idEmployee ")
                    .append("AND r.dayWorking = :dayWorking ")
                    .append("AND r.idRoom not in (SELECT lr.idRoom FROM QLKSLogCleanRoomEntity lr " +
                            "WHERE lr.idEmployee = :idEmployee and lr.dayWorking = :dayWorking and lr.idRoom = r.idRoom ) ");

            log.info("SQL [{}]", hql);

            Query<QLKSCleanScheduleEntity> query = session.createQuery(hql.toString(), QLKSCleanScheduleEntity.class)
                    .setParameter("idEmployee", request.getIdEmployee())
                    .setParameter("dayWorking", java.time.LocalDate.now().toString());
